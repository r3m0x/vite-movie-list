  ...
  const columns: ColumnDef<Movie>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        header: "Seats",
        accessorFn: (row) =>
          `${row.availableSeatsCount}/${row.totalSeatsCount}`,
        cell: (info) => {
          const movie = info.row.original;
          const availablePercentage =
            (movie.availableSeatsCount / movie.totalSeatsCount) * 100;
          return (
            <PercentageBar
              movie={movie}
        ...
        sortingFn: (rowA, rowB) => {
          const percentA =
            rowA.original.availableSeatsCount / rowA.original.totalSeatsCount;
          const percentB =
            rowB.original.availableSeatsCount / rowB.original.totalSeatsCount;
        ...
        <div className="flex flex-wrap gap-6 items-start">
          ...
          <div className="flex-1 min-w-[200px] group">
            <label
              ...
              className="block text-left text-sm font-medium text-gray-700 mb-2 group-hover:text-indigo-600 transition-colors"
            >
              Title
            </label>
            ...
            <div className="relative">
              ...
              >
                ...
                {showTimeOptions.map((showTimeOption) => (
                  <option
                    key={showTimeOption.value}
                    value={showTimeOption.value}
                  >
          ...
          <div className="flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md shadow-sm h-[42px] mt-6 hover:border-indigo-500 transition-colors">
            ...
            />
            <label
              htmlFor="available-seats-filter"
            ...
            {table.getRowCount() === 0 ? (
              ...
              table.getRowModel().rows.map((row) => (
                <tr
                  ...
                  onClick={() => onSelectMovie && onSelectMovie(row.original)}
                >
                  {row.getVisibleCells().map((cell) => (
                    <td
                      key={cell.id}
                      className="px-6 py-4 text-left text-sm text-gray-700 overflow-hidden"
                    >
